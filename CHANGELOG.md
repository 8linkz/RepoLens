...
- `--remote <ssh-target>`, `--remote-key <path>`, and `--remote-label <text>` deploy-mode CLI plumbing. Remote targets accept `host`, `host:port`, `user@host`, or `user@host:port`, default to port `22`, require server deploy mode, reject `--hosted` and Android deploy targets, validate key paths as regular files, and appear in deploy `--dry-run` output. SSH command execution is still reserved for later remote deploy work ([#196](https://github.com/TheMorpheus407/RepoLens/issues/196)).
