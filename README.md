...
# Deploy — preview remote server target metadata
./repolens.sh --project /srv/myapp --agent claude --mode deploy --remote ubuntu@host.example.com:2222 --remote-key ~/.ssh/id_ed25519 --dry-run
...
| `--remote <ssh-target>` | Remote deploy server target metadata. Accepts `host`, `host:port`, `user@host`, or `user@host:port`; only valid with `--mode deploy` server targets; incompatible with `--hosted` and Android deploy targets. The flag is parsed, validated, exported for later remote deploy stages, and shown in `--dry-run`; it does not enable SSH command execution by itself yet. |
| `--remote-key <path>`  | SSH private key path for `--remote`. The path must exist and be a regular file. If omitted, remote SSH stages use normal SSH key resolution when that execution support is enabled. |
| `--remote-label <text>` | Human-readable label for the remote target, parsed for later remote deploy auth prompts. |
...
`--remote <ssh-target>` is accepted only for deploy server targets and may be written as `host`, `host:port`, `user@host`, or `user@host:port`. It is rejected with `--hosted` and Android deploy targets. In this release it only validates and surfaces remote target metadata for follow-up remote deploy stages; it does not yet cause deploy agents to wrap commands in SSH.
...
| `Remote key file does not exist or is not a regular file: …`                                                                         | `--remote-key` points to a missing path or a directory                                                       | Pass an existing private key file path, or omit `--remote-key` to rely on default SSH key resolution in later remote execution stages                                                |
