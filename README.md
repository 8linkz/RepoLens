...
# 6. Full parallel audit (all 290 lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A 290-lens run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 25 code/toolgate domains (220 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 25 code/toolgate domains (220 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 25 code/toolgate domains (220 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 25 code/toolgate domains (220 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (290 total across 29 domains)
...
| **LLM Security** | 3 | LLM output sanitization, rendering safety, prompt injection, RAG/tool/chat-history injection, agent sandbox boundaries, container privilege, subprocess fallbacks, markdown/link injection, external system forwarding, structured output validation, filesystem and command injection risks |
