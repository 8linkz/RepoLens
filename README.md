...
# 6. Full parallel audit (all 288 lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A 288-lens run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 25 code/toolgate domains (218 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 25 code/toolgate domains (218 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 25 code/toolgate domains (218 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 25 code/toolgate domains (218 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (288 total across 29 domains)
...
| **LLM Security** | 1 | LLM output sanitization, rendering safety, markdown/link injection, external system forwarding, structured output validation, filesystem and command injection risks |
