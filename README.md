...
# 6. Full parallel audit (229 audit-visible lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A default 229-lens audit run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
| ------------ | ----------- | ------------------------------------------ | ----------------------------------------------------------------------------- |
| `audit`      | 3×          | 27 code/toolgate/logs domains (229 lenses) | **Default.** Standard code audit — finds issues in existing code              |
| `feature`    | 3×          | 27 code/toolgate/logs domains (229 lenses) | Feature gap discovery — identifies missing capabilities                       |
| `bugfix`     | 3×          | 27 code/toolgate/logs domains (229 lenses) | Bug hunting — finds real bugs and defects                                     |
...
| `custom`     | 1×          | 27 code/toolgate/logs domains (229 lenses) | Change impact analysis — identifies what needs adapting after a change        |
...
## Domains & Lenses (316 total across 32 domains)
...
| **Runtime Log Analysis**     | 2      | Error storm detection for repeated log/event fingerprints above threshold; error cascade reconstruction across components with root cause, chain, terminal symptom, and break-point evidence; `--logs` passes a log file or directory path into lens prompts without orchestrator-level content reads                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
