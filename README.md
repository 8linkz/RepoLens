...
# 6. Full parallel audit (all 281 lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A 281-lens run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
|------|-------------|---------|-------------|
| `audit` | 3× | 24 code/toolgate domains (211 lenses) | **Default.** Standard code audit — finds issues in existing code |
| `feature` | 3× | 24 code/toolgate domains (211 lenses) | Feature gap discovery — identifies missing capabilities |
| `bugfix` | 3× | 24 code/toolgate domains (211 lenses) | Bug hunting — finds real bugs and defects |
...
| `custom` | 1× | 24 code/toolgate domains (211 lenses) | Change impact analysis — identifies what needs adapting after a change |
...
## Domains & Lenses (281 total across 28 domains)
...
| **Kubernetes** | 1 | Pod/container security contexts, Linux capabilities, privilege escalation, host namespaces, hostPath mounts, seccomp, AppArmor, SELinux, Pod Security Standards |
