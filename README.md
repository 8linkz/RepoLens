...
# 6. Full parallel audit (227 audit-visible lenses)
...
> [!NOTE]
> RepoLens generates a lot of automated traffic. A default 227-lens audit run can create dozens to hundreds of remote issues, plus repo reads via `gh`, `tea`, or `fj`, plus parallel AI provider calls.
...
| ------------ | ----------- | ------------------------------------------ | ----------------------------------------------------------------------------- |
| `audit`      | 3×          | 26 code/toolgate domains (227 lenses)      | **Default.** Standard code audit — finds issues in existing code              |
...
| `deploy`     | 1×          | `deployment` domain (26 lenses) or `android` domain (12 lenses) | Server or Android APK audit — inspects a live server or APK target         |
...
## Domains & Lenses (309 total across 31 domains)
...
| **Android**               | `deploy`     | 12 lenses | APK overview, package metadata, bundled dependency CVE inventory, native `.so` inventory, JNI surface, binary hardening, AndroidManifest permissions and component flags, exported IPC components, intent filters, deeplinks, App Links, APK secrets, credentials, internal URLs, WebView security settings, JavaScript bridges, Network Security Config, TLS trust, certificate pinning, MITM traffic observation, logcat sensitive-data leaks, Frida runtime behavior hooks for crypto, file I/O, network, process, reflection, IPC, logging anomalies, and device-aware Android audit context |
