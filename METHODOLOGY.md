...
The current lens inventory spans 32 domains with 316 total lenses, broken down as: 211 code analysis/audit-visible lenses (209 code analysis plus 2 runtime log analysis) + 18 tool gate + 14 product discovery + 43 deployment and Android audit + 13 open-source readiness + 17 content quality.
...
LBA takes the opposite approach. By assigning one prompt per concern (316 total), each lens can devote its full context window and specialization depth to a single domain. The advantages of this decomposition include:
...
|------|---------|---------------|
| **audit** | Find real issues in existing code | 229 (code + toolgate + logs domains) |
| **feature** | Identify missing capabilities | 229 |
| **bugfix** | Find bugs backed by evidence | 229 |
| **custom** | Change impact analysis | 229 |
